...
var comTable =  $('#dataTable').DataTable({
        ajax: '/home/alldatas_tableshow_new',
        columns: [
            { title: "职位名称" },
            { title: "公司名称" },
            { title: "公司类型" },
            ...
            { title: "操作",
              render:function (data, type, full, meta) {
                  var a = $('#user_leval').text();
                  if (a == 1) {
                      return "<button id='info' type='submit' class='btn btn-success' value=" + full[9] + ">详情</button> " +
                          "<button id='delete' type='submit' class='btn btn-primary' value=" + full[9] + ">删除</button>"
                        }
                  else{
                      return "<button id='info' type='submit' class='btn btn-success' value=" + full[9] + ">详情</button> "
     ...
     $.ajax({
        ...
        success:function (data_r) {
            ...
            if (data_res['code'] == 500) {
                ...
                alert('已加入收藏！')
                // window.location.href ='/home/newdata'
            }
        }
    })
});
