...
class LaGoSpider:
    ...
    # 工作职责 # 福利  # 学历 # 薪资 # 工作时间 # 职位描述 # 公司描述 # job 状态 # job 标签
                ...
                if '/' in ''.join(worktime)[:-2]:
                    ...
                    worktimedata = ''.join(worktime)[:-2]
                zh_info = Zhilianjob(platform, ''.join(company_name), ''.join(company_type).strip(), ''.join(company_leavl).strip(), ''.join(job_name),fabu_time , put_time, \
                ...
                db.session.add(zh_info)
                db.session.commit()
                db.create_all()
                print('zhilian insert ok!')
            except:
                print('zhilian error 下一个url!')

# if __name__ == '__main__':
def get_zhilian_data():
    url_list = []
    for i in range(1,30):
